...
namespace bitter {
    ...
    std::ostream& operator<<(std::ostream& o,
                             const const_bit_iterator<BO,UL,EI>& it)
    {
        return o << "[0x" << it.data << ", " << int(it.bitno) << "]";
    }
    template <bit_order BO, typename UL, byte_order EI>
    std::ostream& operator<<(std::ostream& o, const bit_iterator<BO,UL,EI>& it)
    {
        return o << "[0x" << it.data << ", " << int(it.bitno) << "]";
    }
}
...
template <bit_order BO, typename UL, byte_order EI, typename F>
void for_each_bit(F&& f)
{
    std::array<UL,testvec_bits/(8*sizeof(UL))> data = testvec<BO,UL,EI>();
...
template <bitter::bit_order BO, typename UL, byte_order EI, typename F>
...
void observing_tests()
{
    ...
    static constexpr byte_order ei = bit_iterator_traits<Iter>::byte_order;
    static constexpr uint8_t eb = 8 * sizeof(ul); // element bits

    /* Construction and assignment *////////////////////////////////////////////

/*    it("supports construction from byte and bitno", []{
        for_each_bit<bo,ul,ei>([](Data* arr, std::size_t b){
    ...
    it("is swappable", []{
        for_each_bit_pair<bo,ul,ei>([](Data* arr, ptrdiff_t b1, ptrdiff_t b2){
    ...
    */
    ...
    it("is dereferencable with ->", []{
        AssertThat((for_each_bit_check<bo,ul,ei>([](Data* arr, std::size_t b)->bitter::bit {
            Iter i(&arr[b/eb], b%eb);
...
void single_iterator_config_tests()
{
    ...
    describe("const_bit_iterator<bit_order,underlier,endian>", []{
        core_tests<const_bit_iterator<BO,UL,EI>>();
        describe("over const array",
            &observing_tests<const UL,const_bit_iterator<BO,UL,EI>>);
        describe("over mutable array",
            &observing_tests<UL,const_bit_iterator<BO,UL,EI>>);
    });
    describe("bit_iterator<bitter::bit_order BO>", []{
        core_tests<bit_iterator<BO,UL,EI>>();
        observing_tests<UL,bit_iterator<BO,UL,EI>>();
        mutating_tests<bit_iterator<BO,UL,EI>>();
    });
}
...
void single_iterator_one_bitorder_tests()
{
    ...
    describe("over big-endian uint16_t",
        single_iterator_config_tests<BO,uint16_t,byte_order::big_endian>);
    describe("over big-endian uint32_t",
        single_iterator_config_tests<BO,uint32_t,byte_order::big_endian>);
    describe("over big-endian uint64_t",
        single_iterator_config_tests<BO,uint64_t,byte_order::big_endian>);
    describe("over pdp-endian uint32_t",
        single_iterator_config_tests<BO,uint32_t,byte_order::pdp_endian>);
    describe("over little-endian uint16_t",
        single_iterator_config_tests<BO,uint16_t,byte_order::little_endian>);
    describe("over little-endian uint32_t",
        single_iterator_config_tests<BO,uint32_t,byte_order::little_endian>);
    describe("over little-endian uint64_t",
        single_iterator_config_tests<BO,uint64_t,byte_order::little_endian>);
}
